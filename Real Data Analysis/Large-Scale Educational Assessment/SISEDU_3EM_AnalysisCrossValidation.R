...
##   nk[k]  : number of items in testlet k
##   dk[k]  : starting item index (1-based) for testlet k
##   is_HU  : TRUE -> HU (one correlation parameter for testlet k)
##            FALSE -> HT (Toeplitz-by-lag; nk[k]-1 correlation parameters)
K  <- 6
nk <- c(3, 2, 2, 2, 3, 2)
dk <- c(4, 9, 15, 17, 19, 25)
is_HU <- c(FALSE, TRUE, TRUE, TRUE, FALSE, TRUE)

## Build indices for Stan "ragged" rho representation (rho_len and rho_start)
idx <- make_rho_index(nk, is_HU)

## Items outside any testlet (local independence assumed for these)
